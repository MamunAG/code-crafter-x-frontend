export function LoginHero({ apiUrl }: { apiUrl: string }) {
  return (
    <section className="flex flex-col justify-center gap-6">
      ...
      <div className="space-y-4">
        <h1 className="max-w-xl text-4xl font-semibold tracking-tight sm:text-5xl">
          Sign in to continue building the workspace.
        </h1>
        <p className="max-w-xl text-base leading-7 text-slate-600 dark:text-slate-300">
          This form connects to the NestJS auth API at{" "}
          <span className="font-mono text-sm text-slate-900 dark:text-slate-100">
            {apiUrl}
          </span>
          . It sends the backend&apos;s expected login payload and stores the
      ...
      <div className="grid gap-3 text-sm text-slate-600 dark:text-slate-300 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/60 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
          ...
          <div className="font-mono text-xs">/api/v1/auth/login</div>
        </div>
        <div className="rounded-2xl border border-white/60 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
          <div className="font-medium text-slate-900 dark:text-slate-100">Body</div>
          <div className="font-mono text-xs">email, password</div>
        </div>
        <div className="rounded-2xl border border-white/60 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
          <div className="font-medium text-slate-900 dark:text-slate-100">Response</div>
          <div className="font-mono text-xs">access_token, refresh_token</div>
        </div>
      </div>
    </section>
  )
