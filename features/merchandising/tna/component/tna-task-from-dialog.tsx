...
import type { TnaTaskRecord } from "../tna.types"

type TnaTaskFromDialogProps = {
  open: boolean
  apiUrl: string
...
type DeleteTarget = TnaTaskRecord | null
...
export function TnaTaskFromDialog({
  open,
  apiUrl,
  organizationId,
  onOpenChange,
  onTasksChanged,
}: TnaTaskFromDialogProps) {
  const [tasks, setTasks] = useState<TnaTaskRecord[]>([])
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      const message = getErrorMessage(caughtError, "Unable to load TNA tasks right now.")
      setError(message)
      setTasks([])
      toast.error(message)
    } finally {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (editingId) {
        ...
        toast.success("TNA task updated successfully.")
      } else {
        await createTnaTask({
          apiUrl,
          accessToken: token,
        ...
        toast.success("TNA task saved successfully.")
      }
      ...
      const message = getErrorMessage(caughtError, "Unable to save the TNA task right now.")
      setError(message)
      toast.error(message)
    } finally {
      setSaving(false)
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleteTarget(null)
      toast.success("TNA task deleted successfully.")
      await loadTasks()
      await onTasksChanged?.()
    } catch (caughtError) {
      const message = getErrorMessage(caughtError, "Unable to delete the TNA task right now.")
      setError(message)
      toast.error(message)
    } finally {
      setDeleting(false)
  ...
  const columns = useMemo<ColumnDef<TnaTaskRecord>[]>(
    () => [
      {
        id: "serial",
        header: "#",
  ...
  return (
    <>
        ...
        <DialogContent className="left-0 top-0 h-[100dvh] max-h-[100dvh] w-[100vw] max-w-[100vw] translate-x-0 translate-y-0 overflow-hidden rounded-none p-0 sm:left-1/2 sm:top-1/2 sm:h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-2rem)] sm:w-[calc(100vw-2rem)] sm:max-w-5xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg">
          <div className="flex h-full max-h-[100dvh] flex-col sm:max-h-[calc(100vh-2rem)]">
            <div className="border-b border-slate-200/70 px-4 pb-4 pt-5 sm:px-6 sm:pt-6 dark:border-white/10">
              <DialogHeader>
                <DialogTitle>TNA Task Setup</DialogTitle>
                <DialogDescription>Create, update, and delete task options used in TNA detail rows.</DialogDescription>
              </DialogHeader>
            </div>
              ...
              <div className="grid min-h-full gap-4 px-4 py-4 sm:px-6 lg:h-full lg:min-h-0 lg:grid-cols-[320px_minmax(0,1fr)]">
                <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-slate-200/80 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Saved tasks appear in the TNA task combobox.</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="tna-task-name" className="text-xs font-semibold">
                  ...
                  <div className="flex items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-950/40">
                    <Label htmlFor="tna-task-active" className="text-xs font-semibold">
                      Active
                    </Label>
                    <Switch
                      id="tna-task-active"
                ...
                <div className="flex min-h-0 min-w-0 flex-col rounded-lg border border-slate-200/80 bg-white dark:border-white/10 dark:bg-slate-950/40">
                  <div className="flex items-center justify-between gap-3 border-b border-slate-200/70 px-4 py-3 dark:border-white/10">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">{tasks.length} task{tasks.length === 1 ? "" : "s"}</p>
                    </div>
                    <Button type="button" variant="outline" size="icon" className="size-8 rounded-md" onClick={loadTasks} disabled={loading}>
      ...
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(nextOpen) => {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete TNA task</AlertDialogTitle>
            <AlertDialogDescription>
              This will delete <span className="font-medium text-slate-900 dark:text-slate-100">{deleteTarget?.name ?? "this task"}</span> from the saved task list.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
