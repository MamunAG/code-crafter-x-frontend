...
type SizeFieldName = keyof SizeFormValues
...
const SIZE_FIELD_ORDER: SizeFieldName[] = ["sizeName", "isActive"]
...
function buildValidationSummary(errors: FieldErrors<SizeFormValues>) {
  const entries: ValidationSummaryEntry[] = []

  for (const field of SIZE_FIELD_ORDER) {
    ...
    entries.push({
      field,
      label: field === "sizeName" ? "Size name" : "Active",
      message,
    })
  }
...
function getFirstInvalidField(errors: FieldErrors<SizeFormValues>) {
  for (const field of SIZE_FIELD_ORDER) {
    if (errors[field]) {
      return field
    }
...
export function SizeFormDialog({
  ...
  const title = mode === "create" ? "Create size" : "Edit size"
  const description =
    mode === "create"
  ...
  async function handleValidSubmit(values: SizeFormValues) {
    await onSubmit(values)
  }

  function handleInvalidSubmit(formErrors: FieldErrors<SizeFormValues>) {
    ...
    window.requestAnimationFrame(() => {
      document.getElementById(`size-field-${firstInvalidField}`)?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      })
    })
              ...
              {loading ? (
                ...
                <>
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => (
                      <div id="size-field-sizeName" className="space-y-2">
                        ...
                        <Input
                          ...
                          required
                          aria-invalid={Boolean(errors.sizeName)}
                        />
                        <FieldErrorMessage message={getErrorMessage(errors.sizeName?.message)} />
                      </div>
                    )}
                  />
          ...
          <div className="border-t border-slate-200/70 px-6 py-4 dark:border-white/10">
            <DialogFooter>
              ...
              <Button type="submit" disabled={loading || submitting} className="rounded-xl">
                ...
                {mode === "create" ? "Save size" : "Save changes"}
              </Button>
            </DialogFooter>
          </div>
        </form>
