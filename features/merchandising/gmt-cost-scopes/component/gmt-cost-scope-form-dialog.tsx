...
import type { GmtCostScopeFormValues } from "../gmt-cost-scope.types"

type GmtCostScopeEditorMode = "create" | "edit"

type GmtCostScopeFormDialogProps = {
  open: boolean
  mode: GmtCostScopeEditorMode
  loading: boolean
  submitting: boolean
  error: string
  initialValues: GmtCostScopeFormValues
  onOpenChange: (open: boolean) => void
  onSubmit: (values: GmtCostScopeFormValues) => void | Promise<void>
}

type GmtCostScopeFieldName = keyof GmtCostScopeFormValues

type ValidationSummaryEntry = {
  field: GmtCostScopeFieldName
  label: string
  message: string
}

const MOBILE_MAX_SUMMARY_ERRORS = 3
const FIELD_ORDER: GmtCostScopeFieldName[] = ["name", "isActive"]

const gmtCostScopeFormSchema = z.object({
  name: z.string().trim().min(1, "GMT cost scope name is required."),
  isActive: z.boolean(),
})
...
function buildValidationSummary(errors: FieldErrors<GmtCostScopeFormValues>) {
  ...
  for (const field of FIELD_ORDER) {
    ...
    entries.push({
      field,
      label: field === "name" ? "GMT cost scope name" : "Active",
      message,
    })
  }
...
function getFirstInvalidField(errors: FieldErrors<GmtCostScopeFormValues>) {
  for (const field of FIELD_ORDER) {
...
export function GmtCostScopeFormDialog({
  open,
  mode,
  loading,
  submitting,
  ...
  onOpenChange,
  onSubmit,
}: GmtCostScopeFormDialogProps) {
  const isMobile = useIsMobile()
  const title = mode === "create" ? "Create GMT cost scope" : "Edit GMT cost scope"
  const description =
    mode === "create"
      ? "Add a merchandising GMT cost scope master record."
      : "Update the selected merchandising GMT cost scope record."
  ...
  const {
    ...
    reset,
    setFocus,
  } = useForm<GmtCostScopeFormValues>({
    resolver: zodResolver(gmtCostScopeFormSchema),
  ...
  function handleInvalidSubmit(formErrors: FieldErrors<GmtCostScopeFormValues>) {
    const firstInvalidField = getFirstInvalidField(formErrors)
    if (!firstInvalidField) return

    setFocus(firstInvalidField)
    window.requestAnimationFrame(() => {
      document.getElementById(`gmt-cost-scope-field-${firstInvalidField}`)?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      })
    })
              ...
              {loading ? (
                ...
                <>
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => (
                      <div id="gmt-cost-scope-field-name" className="space-y-2">
                        <label htmlFor={field.name} className="text-sm font-medium">
                          GMT cost scope name <span className="text-destructive">*</span>
                        </label>
                        <Input
                          {...field}
                          id={field.name}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <div
                        id="gmt-cost-scope-field-isActive"
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                              Active GMT cost scopes can be used in merchandising records.
                            </p>
                          </div>
                          <Switch
                            id={field.name}
          ...
          <div className="border-t border-slate-200/70 px-6 py-4 dark:border-white/10">
            <DialogFooter>
              ...
              <Button type="submit" disabled={loading || submitting} className="rounded-xl">
                ...
                {mode === "create" ? "Save GMT cost scope" : "Save changes"}
              </Button>
            </DialogFooter>
          </div>
        </form>
