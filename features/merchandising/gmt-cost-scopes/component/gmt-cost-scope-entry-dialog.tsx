...
import type { GmtCostScopeRecord } from "../gmt-cost-scope.types"

type GmtCostScopeEntryDialogProps = {
  open: boolean
  apiUrl: string
  organizationId?: string
  onOpenChange: (open: boolean) => void
  onScopesChanged?: () => void | Promise<void>
}
...
export function GmtCostScopeEntryDialog({
  open,
  apiUrl,
  organizationId,
  onOpenChange,
  onScopesChanged,
}: GmtCostScopeEntryDialogProps) {
  const [scopes, setScopes] = useState<GmtCostScopeRecord[]>([])
  ...
  const loadScopes = useCallback(async () => {
    ...
    try {
      ...
      const message = getErrorMessage(caughtError, "Unable to load GMT cost scopes right now.")
      setError(message)
      setScopes([])
      toast.error(message)
    } finally {
  ...
  useEffect(() => {
    ...
    void loadScopes()
  }, [loadScopes, open, resetForm])

  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const name = formValues.name.trim()
    if (!name) {
      setError("GMT cost scope name is required.")
      return
    }
    ...
    try {
      ...
      if (editingId == null) {
        await createGmtCostScope({
          apiUrl,
          accessToken: token,
          organizationId,
          payload: { name, isActive: formValues.isActive },
        })
        toast.success("GMT cost scope saved successfully.")
      } else {
        await updateGmtCostScope({
          apiUrl,
          accessToken: token,
          organizationId,
          id: editingId,
        ...
        toast.success("GMT cost scope updated successfully.")
      }

      resetForm()
      await loadScopes()
      await onScopesChanged?.()
    } catch (caughtError) {
      const message = getErrorMessage(caughtError, "Unable to save the GMT cost scope right now.")
      setError(message)
      toast.error(message)
    } finally {
      setSaving(false)
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const message = getErrorMessage(caughtError, "Unable to delete the GMT cost scope right now.")
      setError(message)
      toast.error(message)
    } finally {
      setDeleting(false)
        ...
        <DialogContent className="left-0 top-0 h-[100dvh] max-h-[100dvh] w-[100vw] max-w-[100vw] translate-x-0 translate-y-0 overflow-hidden rounded-none p-0 sm:left-1/2 sm:top-1/2 sm:h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-2rem)] sm:w-[calc(100vw-2rem)] sm:max-w-5xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg">
          <div className="flex h-full max-h-[100dvh] flex-col sm:max-h-[calc(100vh-2rem)]">
            <div className="border-b border-slate-200/70 px-4 pb-4 pt-5 sm:px-6 sm:pt-6 dark:border-white/10">
              <DialogHeader>
                <DialogTitle>GMT Cost Scope Setup</DialogTitle>
                <DialogDescription>Create, update, and delete cost scopes used in additional material cost rows.</DialogDescription>
              </DialogHeader>
            </div>
              ...
              <div className="grid min-h-full gap-4 px-4 py-4 sm:px-6 lg:h-full lg:min-h-0 lg:grid-cols-[320px_minmax(0,1fr)]">
                <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-slate-200/80 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div>
                    <p className="text-sm font-semibold text-slate-950 dark:text-slate-50">
                      {editingId == null ? "New cost scope" : "Edit cost scope"}
                    </p>
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                      Active scopes appear in the additional material cost combobox.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="gmt-cost-scope-entry-name" className="text-xs font-semibold">
                      Cost scope name <span className="text-destructive">*</span>
                    </Label>
                    <Input
                      id="gmt-cost-scope-entry-name"
                      value={formValues.name}
                      onChange={(event) => setFormValues((current) => ({ ...current, name: event.target.value }))}
                      placeholder="Input cost scope name"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-950/40">
                    <Label htmlFor="gmt-cost-scope-entry-active" className="text-xs font-semibold">
                      Active
                    </Label>
                    <Switch
                      id="gmt-cost-scope-entry-active"
                ...
                <div className="flex min-h-0 min-w-0 flex-col rounded-lg border border-slate-200/80 bg-white dark:border-white/10 dark:bg-slate-950/40">
                  <div className="flex items-center justify-between gap-3 border-b border-slate-200/70 px-4 py-3 dark:border-white/10">
                    <div>
                      <p className="text-sm font-semibold text-slate-950 dark:text-slate-50">Saved cost scopes</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        {scopes.length} scope{scopes.length === 1 ? "" : "s"}
                      </p>
                    </div>
                    <Button type="button" variant="outline" size="icon" className="size-8 rounded-md" onClick={loadScopes} disabled={loading}>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove <span className="font-medium text-slate-900 dark:text-slate-100">{deleteTarget?.name ?? "this scope"}</span> from new additional material cost selections.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
