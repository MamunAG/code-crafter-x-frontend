...
import { Download, Loader2, Pencil, Plus, RotateCcw, Trash2, Undo2, Upload, X } from "lucide-react"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="overflow-hidden p-0 sm:max-w-4xl">
        <form className="flex max-h-[calc(100vh-2rem)] flex-col" onSubmit={(event) => { event.preventDefault(); onSubmit(draft) }}>
          <div className="border-b border-slate-200/70 px-6 pb-4 pt-6 dark:border-white/10"><DialogHeader><DialogTitle>{mode === "create" ? `Create ${config.singular}` : `Edit ${config.singular}`}</DialogTitle><DialogDescription>{mode === "create" ? `Add a ${config.singular} master record.` : `Update the selected ${config.singular} master record.`}</DialogDescription></DialogHeader>{!loading && error ? <p className="mt-4 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs leading-5 text-red-800 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">{error}</p> : null}</div>
          ...
          <DialogFooter className="border-t border-slate-200/70 px-6 py-4 dark:border-white/10"><Button type="button" variant="outline" className="rounded-xl" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="submit" className="rounded-xl" disabled={disabled || !draft.code.trim() || !draft.name.trim()}>{submitting ? <Loader2 className="size-3.5 animate-spin" /> : null}{mode === "create" ? `Save ${config.singular}` : "Save changes"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  )
...
function RecordsCard({ config, title, records, meta, loading, deleted, page, filters, headerActions, onPage, onEdit, onDelete, onRestore, onPermanent }: { config: MasterDataConfig; title: string; records: MasterDataRecord[]; meta: PaginationMeta | null; loading: boolean; deleted?: boolean; page: number; filters?: ReactNode; headerActions?: ReactNode; onPage: (page: number) => void; onEdit: (id: string) => void; onDelete: (record: MasterDataRecord) => void; onRestore: (record: MasterDataRecord) => void; onPermanent: (record: MasterDataRecord) => void }) {
  const pageSummary = meta?.total ? `Showing ${(meta.page - 1) * meta.limit + 1}-${Math.min(meta.page * meta.limit, meta.total)} of ${meta.total}` : `No ${deleted ? "deleted " : ""}${config.singular} records found`
  const columns = useMemo<ColumnDef<MasterDataRecord>[]>(() => [
    ...
    { id: "name", header: "Name", cell: ({ row }) => <span className="font-medium text-slate-950 dark:text-slate-50">{row.original.name}</span> },
    { id: "nameBn", header: "Bangla name", cell: ({ row }) => <span className="text-slate-600 dark:text-slate-300">{row.original.nameBn || "—"}</span> },
    { id: "status", header: "Status", cell: ({ row }) => <Badge className="rounded-full px-3 py-1" variant={deleted ? "destructive" : row.original.isActive ? "secondary" : "outline"}>{deleted ? "Deleted" : row.original.isActive ? "Active" : "Inactive"}</Badge> },
    { id: "actions", header: "Actions", cell: ({ row }) => <div className="flex justify-end gap-1">{deleted ? <><Button size="icon" variant="ghost" className="rounded-full" title="Restore" onClick={() => onRestore(row.original)}><RotateCcw /></Button><Button size="icon" variant="ghost" className="rounded-full" title="Delete permanently" onClick={() => onPermanent(row.original)}><Trash2 className="text-destructive" /></Button></> : <><Button size="icon" variant="ghost" className="rounded-full" title="Edit" onClick={() => onEdit(row.original.id)}><Pencil /></Button><Button size="icon" variant="ghost" className="rounded-full" title="Delete" onClick={() => onDelete(row.original)}><Trash2 className="text-destructive" /></Button></>}</div> },
  ], [deleted, onDelete, onEdit, onPermanent, onRestore])

  return <AppDataSection title={title} description={pageSummary} data={records} columns={columns} loading={loading} filters={filters} headerActions={headerActions} headerBadges={<><Badge variant="outline" className="w-fit rounded-full px-3 py-1">Page {meta?.totalPages ? meta.page : 0} of {meta?.totalPages ?? 0}</Badge>{deleted ? <Badge variant="outline" className="w-fit rounded-full px-3 py-1">{meta?.total ?? records.length} deleted</Badge> : null}</>} pageSummary={pageSummary} page={page} totalPages={meta?.totalPages ?? 1} onPageChange={onPage} getRowId={(record) => record.id} leadingColumnIds={["code"]} emptyState={<p className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">No records found.</p>} renderMobileItem={(record) => <article className="rounded-2xl border border-slate-200/70 bg-white/90 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.03]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950 dark:text-slate-50">{record.name}</p><p className="truncate font-mono text-xs text-slate-500 dark:text-slate-400">{record.code}</p></div><div className="flex gap-1">{deleted ? <><Button size="icon" variant="ghost" className="rounded-full" title="Restore" onClick={() => onRestore(record)}><RotateCcw /></Button><Button size="icon" variant="ghost" className="rounded-full" title="Delete permanently" onClick={() => onPermanent(record)}><Trash2 className="text-destructive" /></Button></> : <><Button size="icon" variant="ghost" className="rounded-full" title="Edit" onClick={() => onEdit(record.id)}><Pencil /></Button><Button size="icon" variant="ghost" className="rounded-full" title="Delete" onClick={() => onDelete(record)}><Trash2 className="text-destructive" /></Button></>}</div></div><div className="mt-3 flex flex-wrap gap-2"><Badge className="rounded-full px-3 py-1" variant={deleted ? "destructive" : record.isActive ? "secondary" : "outline"}>{deleted ? "Deleted" : record.isActive ? "Active" : "Inactive"}</Badge></div>{record.nameBn ? <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">{record.nameBn}</p> : null}</article>} />
}
...
export function MasterDataWorkspace({ apiUrl, config }: { apiUrl: string; config: MasterDataConfig }) {
  ...
  const [page, setPage] = useState(1); const [deletedPage, setDeletedPage] = useState(1); const [search, setSearch] = useState(""); const [status, setStatus] = useState(""); const [deletedSearch, setDeletedSearch] = useState(""); const [deletedStatus, setDeletedStatus] = useState("")
  ...
  const [recentlyDeleted, setRecentlyDeleted] = useState<MasterDataRecord | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try { const accessToken = token(); const [activeResult, deletedResult] = await Promise.all([
      ...
      listMasterData({ apiUrl, token: accessToken, organizationId, config, page: deletedPage, limit: 5, search: deletedSearch, isActive: deletedStatus, deletedOnly: true }),
  ...
  }, [apiUrl, authFailure, config, deletedPage, deletedSearch, deletedStatus, organizationId, page, search, status])
  ...
  const act = async () => { if (!confirm) return; setWorking(true); try { if (confirm.action === "restore") await restoreMasterData(apiUrl, token(), organizationId, config, confirm.record.id); else await deleteMasterData(apiUrl, token(), organizationId, config, confirm.record.id, confirm.action === "permanent"); if (confirm.action === "delete") setRecentlyDeleted(confirm.record); else if (recentlyDeleted?.id === confirm.record.id) setRecentlyDeleted(null); toast.success(confirm.action === "restore" ? "Record restored successfully." : confirm.action === "permanent" ? "Record deleted permanently." : "Record moved to recently deleted."); setConfirm(null); setRefresh((value) => value + 1) } catch (error) { const message = error instanceof Error ? error.message : "Unable to complete the action."; if (!authFailure(message)) toast.error(message) } finally { setWorking(false) } }
  ...
  const openCreate = () => { setEditorMode("create"); setEditingId(undefined); setEditorValues(defaultValues(config)); setEditorError(""); setEditorOpen(true) }
  const triggerRefresh = () => setRefresh((value) => value + 1)

  return <div className="flex h-full min-h-0 flex-col overflow-hidden"><ScrollArea className="h-full"><div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <Card className="overflow-hidden border-white/60 bg-white/85 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-white/10 dark:bg-slate-950/75"><CardContent className="p-4 sm:p-8 sm:py-2"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-medium uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">HR &amp; Payroll · Core</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{config.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">{config.description}</p><div className="mt-4 flex flex-wrap gap-2"><Badge variant="secondary" className="rounded-full px-3 py-1">{total} total</Badge><Badge variant="outline" className="rounded-full px-3 py-1">{active} active</Badge><Badge variant="outline" className="rounded-full px-3 py-1">{deletedMeta?.total ?? 0} deleted</Badge></div></div><div className="flex flex-col gap-3 sm:flex-row"><AppRefreshButton triggerRefresh={triggerRefresh} title="Refresh" /><AppAddNewButton openCreateDialog={openCreate} title={`New ${config.singular}`} /></div></div></CardContent></Card>

    {recentlyDeleted ? <Card className="border-amber-200 bg-amber-50/80 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10"><CardContent className="p-4 sm:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="space-y-1"><p className="text-sm font-semibold text-amber-950 dark:text-amber-50">Recently deleted {config.singular}</p><p className="text-sm text-amber-900/80 dark:text-amber-100/85">{recentlyDeleted.name} was soft deleted and can still be restored.</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" className="rounded-xl border-amber-300 bg-white/70 text-amber-950 hover:bg-white dark:border-amber-400/40 dark:bg-transparent dark:text-amber-50" onClick={() => setConfirm({ record: recentlyDeleted, action: "restore" })}><Undo2 className="size-3.5" />Restore</Button><Button type="button" variant="destructive" className="rounded-xl" onClick={() => setConfirm({ record: recentlyDeleted, action: "permanent" })}><Trash2 className="size-3.5" />Delete permanently</Button></div></div></CardContent></Card> : null}

    {!organizationId ? <Card className="border-white/60 bg-white/80 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-white/10 dark:bg-slate-950/70"><CardContent className="p-10 text-center text-sm text-slate-500 dark:text-slate-400">Select an organization to manage {config.singular} records.</CardContent></Card> : <>
      <input ref={uploadRef} type="file" accept=".csv,text/csv" className="hidden" onChange={(event) => void upload(event.target.files?.[0])} />
      <RecordsCard config={config} title={`${config.title.replace(" Setup", "")} table`} records={records} meta={meta} loading={loading} page={page} onPage={setPage} onEdit={(id) => void openEdit(id)} onDelete={(record) => setConfirm({ record, action: "delete" })} onRestore={() => undefined} onPermanent={() => undefined} headerActions={<><Button type="button" variant="outline" className="rounded-full" onClick={() => void download()}><Download className="size-3.5" />Template</Button><Button type="button" variant="outline" className="rounded-full" onClick={() => uploadRef.current?.click()}><Upload className="size-3.5" />Upload</Button></>} filters={<AppDataFilterForm fields={[{ id: `${config.slug}-search`, label: "Code or name", kind: "text", value: search, placeholder: "Input code or name", className: "min-w-0 space-y-1 xl:col-span-2", onValueChange: setSearch }, { id: `${config.slug}-status`, label: "Status", kind: "select", value: status || ALL_STATUS_VALUE, placeholder: "All statuses", options: [{ value: ALL_STATUS_VALUE, label: "All statuses" }, { value: "true", label: "Active" }, { value: "false", label: "Inactive" }], onValueChange: (value) => setStatus(value === ALL_STATUS_VALUE ? "" : value) }]} onSubmit={() => setPage(1)} onReset={() => { setSearch(""); setStatus(""); setPage(1) }} onCreate={openCreate} />} />
      <RecordsCard config={config} title={`Deleted ${config.title.replace(" Setup", "").toLowerCase()} records`} records={deleted} meta={deletedMeta} loading={loading} deleted page={deletedPage} onPage={setDeletedPage} onEdit={() => undefined} onDelete={() => undefined} onRestore={(record) => setConfirm({ record, action: "restore" })} onPermanent={(record) => setConfirm({ record, action: "permanent" })} filters={<AppDataFilterForm fields={[{ id: `deleted-${config.slug}-search`, label: "Code or name", kind: "text", value: deletedSearch, placeholder: "Input code or name", className: "min-w-0 space-y-1 xl:col-span-2", onValueChange: setDeletedSearch }, { id: `deleted-${config.slug}-status`, label: "Status", kind: "select", value: deletedStatus || ALL_STATUS_VALUE, placeholder: "All statuses", options: [{ value: ALL_STATUS_VALUE, label: "All statuses" }, { value: "true", label: "Active" }, { value: "false", label: "Inactive" }], onValueChange: (value) => setDeletedStatus(value === ALL_STATUS_VALUE ? "" : value) }]} onSubmit={() => setDeletedPage(1)} onReset={() => { setDeletedSearch(""); setDeletedStatus(""); setDeletedPage(1) }} />} />
    </>}
  </div></ScrollArea>
