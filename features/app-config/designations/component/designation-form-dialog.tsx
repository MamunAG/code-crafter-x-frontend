...
import type { DesignationFormValues } from "../designation.types"
      ...
      <DialogContent className="overflow-hidden p-0 sm:max-w-2xl">
        ...
        >
          <div className="border-b border-slate-200/70 px-6 pb-4 pt-6 dark:border-white/10">
            <DialogHeader>
              <DialogTitle>{mode === "create" ? "Create designation" : "Edit designation"}</DialogTitle>
              <DialogDescription>
                {mode === "create" ? "Add a designation master record." : "Update the selected designation master record."}
              </DialogDescription>
            </DialogHeader>
          ...
          <ScrollArea className="min-h-0 flex-1">
            <div className="space-y-4 px-6 py-5">
              <div className="space-y-2">
                ...
                <Input
                  value={draft.designationName}
                  onChange={(event) => setDraft((current) => ({ ...current, designationName: event.target.value }))}
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Active designations can be selected in HR and payroll records.
                    </p>
                  </div>
                  <Switch
          ...
          <div className="border-t border-slate-200/70 px-6 py-4 dark:border-white/10">
            <DialogFooter>
              ...
              <Button type="submit" disabled={loading || submitting} className="rounded-xl">
                ...
                {mode === "create" ? "Save designation" : "Save changes"}
              </Button>
            </DialogFooter>
          </div>
        </form>
