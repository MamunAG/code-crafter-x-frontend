...
type DeletedDepartmentActionMode = "restore" | "permanent"

type DeletedDepartmentSectionProps = {
  ...
  onOpenAction: (department: DepartmentRecord, mode: DeletedDepartmentActionMode) => void
}
...
function getStatusLabel(department: DepartmentRecord) {
  return department.deleted_at ? "Deleted" : department.isActive === false ? "Inactive" : "Active"
}
...
export function DeletedDepartmentSection({
  data,
  meta,
  page,
  limit,
  ...
  const pageSummary = useMemo(() => {
    if (!meta || meta.total === 0) return "No deleted departments found"
        ...
        <div className="pr-4 text-right">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onSelect={() => onOpenAction(row.original, "restore")}>Restore department</DropdownMenuItem>
    ...
    <Card className="overflow-hidden border-white/60 bg-white/80 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-white/10 dark:bg-slate-950/70">
      <CardHeader className="border-b border-slate-200/70 py-0 dark:border-white/10">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <CardTitle className="text-base">Deleted departments</CardTitle>
      ...
      <CardContent className="p-3 sm:p-0 sm:px-2">
        ...
        >
          <div className="min-w-0 space-y-1 xl:col-span-2">
            <label htmlFor="deletedDepartmentName" className="text-xs font-medium text-slate-700 dark:text-slate-300">Department name</label>
            <Input id="deletedDepartmentName" value={filters.departmentName} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, departmentName: event.target.value })} placeholder="Input department name" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="deletedDepartmentStatus" className="text-xs font-medium text-slate-700 dark:text-slate-300">Status</label>
            <AppSelect
              triggerId="deletedDepartmentStatus"
      ...
      <CardContent className="border-t border-slate-200/70 p-0 dark:border-white/10">
                ...
                <article key={department.id} className="rounded-2xl border border-slate-200/70 bg-white/90 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onSelect={() => onOpenAction(department, "restore")}>Restore department</DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem variant="destructive" onSelect={() => onOpenAction(department, "permanent")}>Delete permanently</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2"><Badge variant={getStatusTone(department)} className="rounded-full px-3 py-1">{getStatusLabel(department)}</Badge></div>
                  <div className="mt-4 space-y-1 text-xs text-slate-500 dark:text-slate-400"><p>Deleted: {formatDate(department.deleted_at)}</p></div>
                </article>
              ))}
            </div>
          ) : (
            <div className="p-4"><EmptyState title="No deleted departments" description="Deleted department records will appear here." /></div>
          )}
        ...
        <div className="hidden lg:block">
          <AppDataTable
            ...
            emptyState={<EmptyState title="No deleted departments" description="Deleted department records will appear here." />}
          />
        </div>
      </CardContent>
    </Card>
