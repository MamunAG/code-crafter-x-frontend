...
import type { PaginationMeta, SupplierFilterValues, SupplierRecord } from "../supplier.types"
...
type SupplierTableSectionProps = {
  ...
  onFilterChange: (nextValues: SupplierFilterValues) => void
...
function getStatusLabel(supplier: SupplierRecord) {
  if (supplier.deleted_at) return "Deleted"
  return supplier.isActive === false ? "Inactive" : "Active"
}

function getStatusTone(supplier: SupplierRecord) {
  if (supplier.deleted_at) return "destructive" as const
  return supplier.isActive === false ? "outline" as const : "secondary" as const
}
...
export function ActiveSuppliersSection({
  data,
  meta,
  page,
  limit,
  ...
  const pageSummary = useMemo(() => {
    if (!meta || meta.total === 0) return "No suppliers found"
  ...
  const columns = useMemo<ColumnDef<SupplierRecord>[]>(() => [
    ...
    },
    {
      id: "created",
      header: "Created",
      cell: ({ row }) => <p className="text-xs text-slate-700 dark:text-slate-200">{formatDate(row.original.created_at)}</p>,
    },
    {
      id: "updated",
      header: "Updated",
      cell: ({ row }) => {
        ...
        const hasUpdateMetadata = Boolean(supplier.updated_by_id || supplier.updated_by_user)

        return hasUpdateMetadata ? (
          <p className="text-xs text-slate-700 dark:text-slate-200">{formatDate(supplier.updated_at)}</p>
        ) : (
          ...
          <div className="pr-4 text-right">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44">
                {canUpdate ? <DropdownMenuItem onSelect={() => onEdit(row.original.id)}>Edit supplier</DropdownMenuItem> : null}
    ...
    <Card className="overflow-hidden border-white/60 bg-white/80 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-white/10 dark:bg-slate-950/70">
      <CardHeader className="border-b border-slate-200/70 py-0 dark:border-white/10">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <CardTitle className="text-base">Suppliers table</CardTitle>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button type="button" variant="outline" size="icon-sm" className="rounded-full">
                    <MoreHorizontal className="size-3.5" />
                    <span className="sr-only">Open supplier bulk actions</span>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem onSelect={onUploadTemplate} disabled={uploadingTemplate}>
                    ...
                    {uploadingTemplate ? "Uploading suppliers..." : "Upload suppliers"}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ) : null}
      ...
      <CardContent className="p-3 sm:p-0 sm:px-2">
        ...
        >
          ...
          <div className="min-w-0 space-y-1">
            ...
            <Input id="filterSupplierCode" value={filters.code} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, code: event.target.value })} placeholder="Input code" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="filterSupplierContact" className="text-xs font-medium text-slate-700 dark:text-slate-300">Contact</label>
            <Input id="filterSupplierContact" value={filters.contact} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, contact: event.target.value })} placeholder="Input contact" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="filterSupplierEmail" className="text-xs font-medium text-slate-700 dark:text-slate-300">Email</label>
            <Input id="filterSupplierEmail" value={filters.email} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, email: event.target.value })} placeholder="Input email" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="filterSupplierStatus" className="text-xs font-medium text-slate-700 dark:text-slate-300">Status</label>
            <AppSelect
              triggerId="filterSupplierStatus"
        ...
        <div className="lg:hidden">
              ...
              {data.map((supplier) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {canUpdate || canDelete ? (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-44">
                          {canUpdate ? <DropdownMenuItem onSelect={() => onEdit(supplier.id)}>Edit supplier</DropdownMenuItem> : null}
                          {canUpdate && canDelete ? <DropdownMenuSeparator /> : null}
                          {canDelete ? <DropdownMenuItem variant="destructive" onSelect={() => onDelete(supplier)}>Delete supplier</DropdownMenuItem> : null}
                        </DropdownMenuContent>
                      </DropdownMenu>
                    ) : null}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Badge variant={getStatusTone(supplier)} className="rounded-full px-3 py-1">{getStatusLabel(supplier)}</Badge>
                    <Badge variant="outline" className="rounded-full px-3 py-1">{supplier.code || "No code"}</Badge>
                  </div>

                  <div className="mt-4 space-y-1 text-xs text-slate-500 dark:text-slate-400">
                    <p>Contact: {supplier.contact || "-"}</p>
                    <p>Email: {supplier.email || "-"}</p>
                    <p>Created: {formatDate(supplier.created_at)}</p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200/70 px-4 py-4 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Button type="button" variant="outline" size="icon-sm" className="rounded-xl" onClick={() => onPageChange((current) => Math.max(1, current - 1))} disabled={loading || page <= 1}>
                <ChevronLeft className="size-3.5" />
              </Button>
              <Button type="button" variant="outline" size="icon-sm" className="rounded-xl" onClick={() => onPageChange((current) => Math.min(meta?.totalPages ?? 1, current + 1))} disabled={loading || page >= (meta?.totalPages ?? 1)}>
