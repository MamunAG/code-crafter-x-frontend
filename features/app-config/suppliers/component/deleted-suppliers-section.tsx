...
type DeletedSupplierActionMode = "restore" | "permanent"

type DeletedSuppliersSectionProps = {
  data: SupplierRecord[]
  ...
  onOpenAction: (supplier: SupplierRecord, mode: DeletedSupplierActionMode) => void
  canRestore: boolean
...
function getStatusLabel(supplier: SupplierRecord) {
  return supplier.deleted_at ? "Deleted" : supplier.isActive === false ? "Inactive" : "Active"
}
...
export function DeletedSuppliersSection({
  data,
  meta,
  page,
  limit,
  ...
  const deletedPageSummary = useMemo(() => {
    if (!meta || meta.total === 0) return "No deleted suppliers found"
          ...
          <div className="pr-4 text-right">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                {canRestore ? <DropdownMenuItem onSelect={() => onOpenAction(row.original, "restore")}>Restore supplier</DropdownMenuItem> : null}
    ...
    <Card className="overflow-hidden border-white/60 bg-white/80 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-white/10 dark:bg-slate-950/70">
      <CardHeader className="border-b border-slate-200/70 py-0 dark:border-white/10">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <CardTitle className="text-base">Deleted suppliers</CardTitle>
      ...
      <CardContent className="p-3 sm:p-0 sm:px-2">
        ...
        >
          ...
          <div className="min-w-0 space-y-1">
            ...
            <Input id="deletedSupplierCode" value={filters.code} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, code: event.target.value })} placeholder="Input code" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="deletedSupplierContact" className="text-xs font-medium text-slate-700 dark:text-slate-300">Contact</label>
            <Input id="deletedSupplierContact" value={filters.contact} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, contact: event.target.value })} placeholder="Input contact" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="deletedSupplierEmail" className="text-xs font-medium text-slate-700 dark:text-slate-300">Email</label>
            <Input id="deletedSupplierEmail" value={filters.email} className="h-7 rounded-md px-2 text-xs" onChange={(event) => onFilterChange({ ...filters, email: event.target.value })} placeholder="Input email" />
          </div>
          <div className="min-w-0 space-y-1">
            <label htmlFor="deletedSupplierStatus" className="text-xs font-medium text-slate-700 dark:text-slate-300">Status</label>
            <AppSelect
              triggerId="deletedSupplierStatus"
      ...
      <CardContent className="border-t border-slate-200/70 p-0 dark:border-white/10">
        {error ? (
          <div className="p-4">
            <EmptyState title="Unable to load deleted suppliers" description={error} />
          </div>
        ) : null}
          ...
          <>
            <div className="lg:hidden">
              {loading ? (
                  ...
                  {data.map((supplier) => (
                    ...
                    >
                        ...
                        {canRestore || canPermanentlyDelete ? (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              {canRestore ? <DropdownMenuItem onSelect={() => onOpenAction(supplier, "restore")}>Restore supplier</DropdownMenuItem> : null}
                              {canRestore && canPermanentlyDelete ? <DropdownMenuSeparator /> : null}
                              {canPermanentlyDelete ? (
                                <DropdownMenuItem variant="destructive" onSelect={() => onOpenAction(supplier, "permanent")}>
                                  Delete permanently
                                </DropdownMenuItem>
                              ) : null}
                            </DropdownMenuContent>
                      ...
                      <div className="mt-4 space-y-1 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <p>Deleted: {formatDate(supplier.deleted_at)}</p>
                      </div>
                    </article>
                  ))}
                </div>
                ...
                <div className="p-4">
                  <EmptyState
                    title="No deleted suppliers found"
