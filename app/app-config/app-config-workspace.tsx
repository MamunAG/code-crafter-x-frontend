...
function getNavItemStateClass(isActive: boolean, isExactActive: boolean) {
  if (isExactActive) {
    return "bg-sky-100 text-sky-950 font-medium hover:bg-sky-100 dark:bg-white/10 dark:text-white dark:hover:bg-white/10"
  }
...
export function AppConfigWorkspace({ children }: { children?: ReactNode }) {
  const pathname = usePathname()
  const appModule = getModuleNavigation("app-config")
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="flex items-start justify-between gap-2 border-b border-slate-200 px-4 py-4 dark:border-white/10">
          {!isCompact ? (
            ...
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-sky-600 text-xs font-semibold text-white">
              A
            </div>
          )}
  ...
  return (
    <div
      className={cn(
        "relative flex h-full min-h-0 w-full flex-1 flex-col px-3 pb-4 pt-0 sm:px-4 sm:py-6 lg:flex-row",
    ...
    >
      <button
        ...
        aria-label="Open app configuration navigation"
      >
      ...
      <aside
        className={cn(
          "hidden h-full min-h-0 w-full shrink-0 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white/70 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/70 lg:flex lg:rounded-r-lg",
          collapsed ? "lg:pointer-events-none lg:w-0 lg:overflow-hidden lg:border-r-0" : "lg:w-64",
        )}
      >
      ...
      >
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{appModule.label}</SheetTitle>
            <SheetDescription>{appModule.description}</SheetDescription>
          </SheetHeader>
          {sidebarContent(true)}
        </SheetContent>
      </Sheet>
